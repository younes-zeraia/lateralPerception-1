...
function clusteringResults = NCapRoadEdgeProcess(lineTypeMes,nextLineTypeMes,lineTypeGT,lineOffsetMes,nextLineOffsetMes,measureQuality,timeArray,param)
    ...
    %% CLUSTERING
    clusteringResults.rightRoadEdgeFNRatio          = getFNRatio(lineTypeMes==param.roadEdge,lineTypeGT==param.roadEdge);
    clusteringResults.rightRoadEdgeFPRatio          = getFPRatio(lineTypeMes==param.roadEdge,lineTypeGT==param.roadEdge);
    clusteringResults.rightRoadEdgeHITRatio         = getHITRatio(lineTypeMes==param.roadEdge,lineTypeGT==param.roadEdge);
    clusteringResults.nextRightRoadEdgeFNRatio      = getFNRatio(nextLineTypeMes==param.roadEdge,lineTypeGT==param.roadEdge);
    clusteringResults.nextRightRoadEdgeHITRatio     = getHITRatio(nextLineTypeMes==param.roadEdge,lineTypeGT==param.roadEdge);
