function [YLineLeftInt,YLineLeftExt,YLineRightInt,YLineRightExt,lineTypeLeft,lineTypeRight,yRefreshedLeft,yRefreshedRight] = detectLineTopView_VBOX(videosFolder,videoName,cropWindowLeft,cropWindowRight,pointWheelLeft_px,pointWheelRight_px,CalibLeft,CalibRight,ThresholdCoef,resizeWindow,VehWidth)
    dispPlot = 0;
    savePlot = 0;
% This script intends to detect lines from "Top wheels" Point of view
    ...
    pointWheelLeft_mm = pointsToWorld(camParams,RotMatLeft,TransMatLeft,[pointWheelLeft_px(1) pointWheelLeft_px(2)]);
    pointWheeRight_mm = pointsToWorld(camParams,RotMatRight,TransMatRight,[pointWheelRight_px(1) pointWheelRight_px(2)]);
    %% Path
    initPath     = pwd;
    ...
    l = 1;
    tic
    nFrames = ceil(video.Duration*video.FrameRate)-1;
    ...
    if savePlot == 1
        plotVid = VideoWriter([videosFolder '\' erase(erase(videoName,'.mp4'),'.avi') '_processed.avi']);
    ...
    while video.hasFrame
        ...
        [ImLeft,ImUndistortLeft,levelLeft] = filterImageMDE(IOrig,cropWindowLeft,resizeWindow,camParams,levelLeft,ThresholdCoef,SE,kernel);
        [ImRight,ImUndistortRight,levelRight] = filterImageMDE(IOrig,cropWindowRight,resizeWindow,camParams,levelRight,ThresholdCoef,SE,kernel);
        ...
        % Fit lines (2 edges)
        [linesLeft  hLeft_2a hLeft_2b hLeft_3a hLeft_3b] = getFittedLines(EdgesLeft,nEdgesLeft,camParams,pointWheelLeft_px,RotMatLeft,TransMatLeft,hLeft_2a,hLeft_2b,hLeft_3a,hLeft_3b,VehWidth/2,dispPlot);
        [linesRight hRight_2a hRight_2b hRight_3a hRight_3b] = getFittedLines(EdgesRight,nEdgesRight,camParams,pointWheelRight_px,RotMatRight,TransMatRight,hRight_2a,hRight_2b,hRight_3a,hRight_3b,-VehWidth/2,dispPlot);
...
%% FUNCTIONS
function [ImOut,I4,level] = filterImageMDE(ImIn,cropWindow,resizeWindow,camParams,level,ThresholdCoef,SE,kernel)
    ...
    I15 = bwareaopen(imdilate(imerode(edge(I9,'canny'),SE.I13),SE.I13),40);
...
%% Get fitted lines
function [lines,h_2a,h_2b,h_3a,h_3b] = getFittedLines(Edges,nEdges,camParams,pointWheel_px,RotMat,TransMat,h_2a,h_2b,h_3a,h_3b,offsetVehWidth,dispPlot)
    ...
    if nEdges > 0
        ...
        [line1,h_3a,h_2a] = fitLine(Edges(1).x,Edges(1).y,Edges(2).x,Edges(2).y,camParams,pointWheel_px,RotMat,TransMat,h_3a,h_2a,offsetVehWidth,dispPlot);

        if nEdges > 2
            % 2nd order fit on second line (2 shortest edges among the 4 detected)
            [line2,h_3b,h_2b] = fitLine(Edges(3).x,Edges(3).y,Edges(4).x,Edges(4).y,camParams,pointWheel_px,RotMat,TransMat,h_3b,h_2b,offsetVehWidth,dispPlot);
        else
            lineB.valid = false;
        end
    end
...
function [lineFit,h_3,h_2] = fitLine(xL1_px,yL1_px,xL2_px,yL2_px,camParams,pointWheel_px,RotMat,TransMat,h_3,h_2,offsetVehWidth,dispPlot)
