% This function computes the FN ratio of a measured signal
% given its GroundTruth signal.
%
%       (n_Samples_Measure == 0 AND n_Samples_GroundTruth == 1)
% FN% = -------------------------------------------------------
%                    n_Samples_GroundTruth == 1

function FNRatio = getFNRatio(measureFlag,GroundTruthFlag)
    %% Format check
    ...
    %% Process
    FNRatio = sum(~measureFlag & GroundTruthFlag,1)/sum(GroundTruthFlag,1);
end