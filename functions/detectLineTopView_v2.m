function [YLine1 YLine2] = detectLineTopView(videosFolder,videoName,cropWindow,pointWheel_px,Calib,ThresholdCoef,resizeWindow)
    dispPlot = 1;
% This script intends to detect lines from "Top wheels" Point of view
    Ry180       = [-1 0 0; 0 1 0;0 0 -1];
    Rz90        = [ 0 -1 0; 1 0 0;0 0 1];
%     RotMat     = Rz90*Ry180*RotMatrix;
    camParams   = Calib.camParams;
    RotMat      = Calib.R;
    TransMat    = Calib.t;
    
    pointWheel_mm = pointsToWorld(camParams,RotMat,TransMat,[pointWheel_px(1) pointWheel_px(2)]);
    imageBorder_px= [1 1;1 resizeWindow(1);resizeWindow(2) resizeWindow(1);resizeWindow(2) 1];
    imageBorder_mm= pointsToWorld(camParams,RotMat,TransMat,[imageBorder_px(:,2) imageBorder_px(:,1)])-pointWheel_mm;
    imageBorder_mm= [imageBorder_mm(:,1) -imageBorder_mm(:,2)];
    %% Path
    initPath     = pwd;
...
%             dispPlot = 1;
        ...
        I8  = imdilate(I7,SE.I8);
        I9  = conv2(single(I8), kernel, 'same') > 0.5; % Rethreshold
        I10 = edge(I9,'canny');
%         I11 = imdilate(I10,strel('disk',3));
%         I12 = bwmorph(I11,'thin',inf);
        I13 = imerode(I10,SE.I13);
        I14 = imdilate(I13,SE.I14);
        I15 = bwareaopen(I14,40);
        I16 = imdilate(I15,SE.I16);
        I17 = bwmorph(I16,'thin',inf);
        I18 = bwareafilt(I17,4);
        I19 = bwlabel(I18);
        [lines(1).x lines(1).y] = find(I19==1);
        [lines(2).x lines(2).y] = find(I19==2);
        [lines(3).x lines(3).y] = find(I19==3);
        [lines(4).x lines(4).y] = find(I19==4);
        ...
        line1.detected   = isDetectedLine(xL1_px,yL1_px);
        line2.detected   = isDetectedLine(xL2_px,yL2_px);
        line3.detected   = isDetectedLine(xL3_px,yL3_px);
        line4.detected   = isDetectedLine(xL4_px,yL4_px);
        ...
        if nLines > 0
            ...
            lineA = isValidLine(line1,line2);
            
            if nLines > 2
                pointsLine3 = pointsToWorld(camParams,RotMat,TransMat,[yL3_px xL3_px]);
                pointsLine4 = pointsToWorld(camParams,RotMat,TransMat,[yL4_px xL4_px]);

                xL3_mm      = pointsLine3(:,1)-pointWheel_px(1);
                yL3_mm      = -(pointsLine3(:,2)-pointWheel_px(2));
                xL4_mm      = pointsLine4(:,1)-pointWheel_px(1);
                yL4_mm      = -(pointsLine4(:,2)-pointWheel_px(2));

                poly3       = polyfit(xL3_mm,yL3_mm,2);
                poly4       = polyfit(xL4_mm,yL4_mm,2);

                line3 = lengthPosLine(xL3_mm,yL3_mm);
                line4 = lengthPosLine(xL4_mm,yL4_mm);
                lineB = isValidLine(line3,line4);
            
                beta   = [beta
                          atan([poly3(2);poly4(2)])];
            ...
            if (lineA.valid || nLines > 2 && lineB.valid) && dtle_AB>prevDtle + 200  && prevDtle > 100
                k_switch = k_switch + 1;
            ...
            if dispPlot==1
                ...
                if nPlot == 0
                    h_3(1) = animatedline(0,0,'Marker','+','Color','r','LineWidth',2,'Parent',axes3);
                    h_3(2) = animatedline('LineWidth',2,'Color','blue','Parent',axes3);
                    h_3(3) = animatedline('LineWidth',2,'Color','red','Parent',axes3);
                    h_3(4) = animatedline('LineStyle','--','Color','black','LineWidth',2,'Parent',axes3);
                    h_3(5) = animatedline('LineStyle','--','Color','black','LineWidth',2,'Parent',axes3);
                    h_3(6) = animatedline('LineStyle','none','Marker','+','Color','black','LineWidth',2,'Parent',axes3);
                    
                    h_2(1) = animatedline('LineWidth',2,'Color','blue','Parent',axes2);
                    h_2(2) = animatedline('LineWidth',2,'Color','red','Parent',axes2);
                    
                    h_3(7) = animatedline('LineWidth',2,'Color','green','Parent',axes3);
                    h_3(8) = animatedline('LineWidth',2,'Color','yellow','Parent',axes3);
                    h_3(9) = animatedline('LineStyle','--','Color','black','LineWidth',2,'Parent',axes3);
                    h_3(10) = animatedline('LineStyle','--','Color','black','LineWidth',2,'Parent',axes3);
                    h_3(11) = animatedline('LineStyle','none','Marker','+','Color','black','LineWidth',2,'Parent',axes3);
                    
                    h_2(3) = animatedline('LineWidth',2,'Color','green','Parent',axes2);
                    h_2(4) = animatedline('LineWidth',2,'Color','yellow','Parent',axes2);
                    
%                     delete(h_3);
                end
%                 h_3(1) = plot(0,0,'+r','LineWidth',2,'Parent',axes3);
                clearpoints(h_3(2));
                clearpoints(h_3(3));
                clearpoints(h_3(4));
                clearpoints(h_3(5));
                ...
                if nLines > 0
                    ...
                    if lineA.valid == true
                        ...
                        addpoints(h_3(2),xL1_mm,yL1_mm);
                        addpoints(h_3(3),xL2_mm,yL2_mm);
                        addpoints(h_3(4),xfit1_mm,yfit1_mm);
                        addpoints(h_3(5),xfit2_mm,yfit2_mm);
                        addpoints(h_3(6),xp(1:2),yp(1:2));
                        
                        addpoints(h_2(1),yL1_px,xL1_px);
                        addpoints(h_2(2),yL2_px,xL2_px);
                        
                        uistack(h_2(1),'top') %you can also do uistack(b,'bottom')
                        uistack(h_2(2),'top') %you can also do uistack(b,'bottom')
                    end
                    
                    if nLines > 2 && lineB.valid == true
                        xfit3_mm    = [xL3_mm(1):1:xL3_mm(end)];
                        xfit4_mm    = [xL4_mm(1):1:xL4_mm(end)];

                        yfit3_mm     = polyval(poly3,xfit3_mm);
                        yfit4_mm     = polyval(poly4,xfit4_mm);
                        
                        addpoints(h_3(7),xL3_mm,yL3_mm);
                        addpoints(h_3(8),xL4_mm,yL4_mm);
                        addpoints(h_3(9),xfit3_mm,yfit3_mm);
                        addpoints(h_3(10),xfit4_mm,yfit4_mm);
                        addpoints(h_3(11),xp(3:4),yp(3:4));
                        
                        addpoints(h_2(3),yL3_px,xL3_px);
                        addpoints(h_2(4),yL4_px,xL4_px);
                        
                        uistack(h_2(3),'top') %you can also do uistack(b,'bottom')
                        uistack(h_2(4),'top') %you can also do uistack(b,'bottom')
                    end
                end
            end
...
%% FUNCTIONS
function ImOut = filterImageMDE(ImIn,cropWindow,resizeWindow,camParams,level,ThresholdCoef,SE,kernel)
